    ...
    async function deliverFinal(state) {
        ...
        try {
            if (state.status === 'done') {
                ...
                const authRefreshCanRetry = state.authRefresh && (
                    state.authRefresh.ok || /invalid_grant|No quota-eligible|нет свободного|ниже резерв/i.test(authRefreshText)
                );
                if (!quotaNotice && authRouteError && state.authRouteBlock && state.authRouteBlock.ok && authRefreshCanRetry && !state.authRetryOf) {
                    const retryNote = /No quota-eligible|нет свободного|ниже резерв/i.test(authRefreshText)
                        ? '🔁 Авторизация выбранного Claude-маршрута сломалась. Проверяю ротацию и реальные лимиты…'
                        : '🔁 Авторизация выбранного Claude-маршрута сломалась. Отключаю маршрут и повторяю запрос автоматически…';
                    mergeJobState(jobId, {
                ...
                if (!quotaNotice && transientApiError && state.sessionId) {
                    ...
                    } else if (state.authRefresh && state.authRefresh.ok === false && /invalid_grant/i.test(authRefreshText)) {
                        hint = '\n\nℹ OAuth refresh token этого Claude-маршрута больше не принимается. Маршрут отключён; нужен новый full OAuth login/credentials для этого аккаунта.';
                    } else {
