...
function buildSystemPrompt(config, settings) {
    ...
    const liveInputPath = path.join(jobDir, 'live-inputs.jsonl');
    parts.push(
        'LIVE-ДОПОЛНЕНИЯ ОТ ПОЛЬЗОВАТЕЛЯ. Пока задача выполняется, пользователь может нажать кнопку «в текущую работу». ' +
        'Такие сообщения бот пишет в live-inbox: `' + liveInputPath + '`. ' +
        'Перед финальным ответом, после долгих ожиданий и перед важными необратимыми действиями проверяй этот файл командой вроде `test -s ' + liveInputPath + ' && tail -n 20 ' + liveInputPath + '`. ' +
        'Если появились новые строки, учитывай их как рекомендации/уточнения к текущей задаче, а не как отдельный новый запрос.'
    );
